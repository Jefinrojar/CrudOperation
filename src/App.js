      ...
      <div className="container">
          ...
          <tbody>
            {userdata.length > 0 && userdata.map((user, index) => {
              return (<tr>
                ...
                <td>{user.address}</td>
                <td>
                  <button className='btn1' onClick={() => editUser(index)}><Edit size={16}></Edit><span>Edit</span></button>&nbsp;
                  <button className='btn2' onClick={() => deleteUser(index)}><Trash2 size={16}></Trash2><span>Delete</span></button>
                </td>
              </tr>)
            })
            }
        ...
        <Modal open={open} onClose={onCloseModal} center>
          ...
          <div className='form'>
            ...
            <input type="tel" pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" value={user.phone} onChange={(e) => setUser({ ...user, "phone": e.target.value })} required/>
            <label htmlFor="">Address</label>
            <textarea name="" id="" value={user.address} cols="30" rows="5" onChange={(e) => setUser({ ...user, "address": e.target.value })} required></textarea>
